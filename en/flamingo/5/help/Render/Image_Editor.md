 ...
 * Adjust the [tone operator](Image_Editor.htm#Tone_mapping) settings.
 * Change the intensity of any lighting channel.
 * Add special image-based effects: [Haze](Image_Editor.htm#Haze), [Depth Blur](Image_Editor.htm#Depth_Blur), and [Glare](Image_Editor.htm#Glare).
 *  [Save](Image_Editor.htm#Save_Tonemapped_Image_As) a tone-mapped image in a bitmap format such as .jpg or .png.
 * Save the luminance information to an [HDR format](Image_Editor.htm#Save_HDR_Image_As).
 * View and save additional masked channels ( [alpha](Image_Editor.htm#Alpha_Channel), [distance](Image_Editor.htm#Distance_Channel), [material](Image_Editor.htm#Material_Channel) ) for use in advanced compositing.
 * Save a [Piranesi©](http://www.piranesi.co.uk/) file format (*.epx) which can be used to create non photo-realistic rendering.
 * Use Image [Arithmetic](Image_Editor.htm#Arithmetic) for tasks such as stitching together an image that has been generated by separate nodes on the render farm.
 * Save the [Lighting Settings](Image_Editor.htm#Save_Lighting_Settings_As) used to generate this rendering. These Lighting Settings can then be used to generate more renderings.
...
Saves the [lighting scheme](..\Lighting\Lighting_Tab.htm#Open_lighting_scheme).
...
Allows piecing together or overlaying segments of images rendered using the [Render Farm Single Image](Automate_Rendering.htm#Single_Images) function.
...
Displays the [material mask](Image_Editor.htm#Material_Channel).
...
1.
 ...
 [Save](Render_Window.htm#Export_to_nXtImage) your rendering results as an. **nXtImage**.
...
See [Render Window: Brightness](Render_Window.htm#Brightness).


### Burn
{: .toc-header }

See [Render Window: Burn](Render_Window.htm#Burn) 


### Saturation
{: .toc-header }

See [Render Window: Saturation](Render_Window.htm#Saturation) 


### Histogram
{: .toc-header }

See [Render Window: Histogram](Render_Window.htm#Histogram) 
...
Blur pixels both in front and behind the **In-Focus Zone** range. This is a quick way to get a depth-of-field effect.&#160;It is not as accurate as using the built-in pre-render [Depth of Field](Render_Tab.htm#DepthOfFieldOption).
