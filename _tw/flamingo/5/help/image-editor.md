...
### 儲存遮罩
{: #save-mask}
nXtImage 檔案內含三種通道資訊，可在大部分的平面繪圖軟體裡做為影像合成的遮罩使用，通道是儲存在每個像素的額外資訊，以灰階值表示，每個通道都可以獨立檢視或另存圖片。

##### 附註：

 1. Alpha 通道可儲存至支援 Alpha 通道的圖片格式。
...
#### 結合路徑追蹤
結合使用路徑追蹤引擎彩現的影像，例如：十張以 20 個處理數彩現的影像可結合成為一張處理數為 200 的彩現影像。
...
### 景深模糊
{: #depth-blur}
彩現影像裡的每個像素都含有距離通道，該通道的數值代表一個像素距離攝影機的遠近，可用來產生景深模糊的效果。
...
#### 清晰範圍
{: #in-focus-zone}
**焦點**前、後影像未做模糊處理的範圍，單位為公尺，超過此範圍的影像的模糊度會隨著距離提高，模擬真實攝影的景深。
...
#### 兩者
**清晰範圍**前後都做模糊處理，此種後處理效果的景深模糊不如彩現時直接運算的[景深](render-tab.html#depthoffieldoption)模糊精確。

### 光芒
{: #glare}
以像素的亮度值決定是否在該像素周圍產生光芒效果，彩現影像裡只有高亮度的像素會有這種效果。
在彩現影像上移動滑鼠游標可顯示像素的亮度值。
...
#### 臨界值
數值越低作用的範圍越大，所有亮度高於臨界值的像素都會產生光芒效果。
