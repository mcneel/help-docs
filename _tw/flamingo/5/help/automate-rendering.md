...
## 批次彩現
{: #batch-rendering}
批次彩現可設定一個工作清單，自動連續彩現數個場景。批次工作可設定要彩現的視圖、解析度、處理數。批次工作可在本機彩現，或傳送到[彩現農場](render-farm.html)，批次彩現的工作清單會儲存在模型裡。

##### 可以在哪找到這個指令?

 * 功能表 > Flamingo nXt 5.0 > 其它工具 > 批次彩現
...
#### 刪除
選取一個批次工作，按**刪除**，將此工作從工作清單移除。

#### 內容
選取一個批次工作，按**內容**，開啟[批次彩現內容](#batch-render-properties)對話框，可設定輸出的**檔案名稱**、**解析度**、**處理數**限制。
...
#### 彩現的作業視窗
顯示批次工作彩現的視圖，進一步的說明請參考：[彩現選項 > 彩現的作業視窗](render-tab.html#viewtorender)
...
預設為使用目前文件的解析度設定彩現，取消勾選此核取方塊可自訂彩現解析度，進一步的說明請參考：[彩現選項 > 彩現解析度](render-tab.html#resolution)
