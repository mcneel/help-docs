...
> **Flamingo nXt 5.0**メニューで**その他のツール &gt; Flamingo nXtイメージエディタ**をクリックします。
...
##### メモ:
 ...
 1. アルファチャンネルは、トーンマッピングされたイメージを保存時にアルファを含むファイル形式を選択することで、トーンマッピングされたイメージに含むことができます。
 1. 距離およびマテリアルチャンネルはアンチエイリアスされておらず、ハードなエッジのアーチファクトを表示することがあります。使用前にマスクに少量のガウスブラー（ぼかし）を追加すると、それらのエッジをソフトにすることができることがあります。
 1. マテリアルチャンネルは、255の異なるマテリアルを一意的に符号化するだけです。モデルにそれより多くのマテリアルが含まれている場合、マスク色の中には繰り返されるものもあります。
...
*20のパスでレンダリング（左）、10枚の20パスのイメージを結合してできた200パスのイメージ（右）*
...
##### イメージの効果をアニメートするには
 ...
 1. 最初のイメージをセットアップします。**フレーム**編集ボックスの横にある**プラス (+)**ボタンをクリックします。
...
## FX設定
特殊なエフェクトをイメージに追加できます。これらのエフェクトの多くは、NXTimage形式が格納するより多くの情報を使用します。例えば、グレアは輝度空間を使用して実際の光の値に影響を与え、ヘイズはイメージの中の距離を使用します。
